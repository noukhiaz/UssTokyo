...
<?php if(isset($_GET['edit'])){echo "Edit brand";}else{echo 'Add new brands';}?>
...
<select class="form-control" name="parent_company" required>
	<option value="">Select...</option>
	<?php if(isset($_GET['edit'])){
	$parent_company = $row['parent_company'];
	$companies_id = mysql_query("select * from companies where id = '$parent_company' ");
	$rows = mysql_fetch_assoc($companies_id);
	?>
	<option value="<?php echo $rows['id'];?>" selected="selected"><?php echo $rows['name'];?></option>
    <option value="">-----------------</option>
	<?php }?>
	<?php 
	$region = mysql_query("select * from companies where is_active = '1' order by name Asc");
