...
*/
			 ...
			 if((isset($_REQUEST['grade_from']) && isset($_REQUEST['mileage_to'])))
               {
				...
				//	echo 'khan';
					$WHERE[] = "RATE BETWEEN CAST('$grade_from' AS DECIMAL) AND CAST('$grade_to' AS DECIMAL)";
                    //$WHERE[] = "RATE BETWEEN '$grade_from' AND '$grade_to'";
					}	
...
//echo 'Lot'. $lot;
		...
		$get_cars_counterss = aj_get("select * from $table where $WHERE"); // 1=>debug  
                ...
                if (!$get_cars) { $get_cars = []; }
                 foreach($get_cars_counterss as $va)
                  ...
                  echo '<h4>Total <span style="color:#c20003">'.$aa . '</span> records found for <span style="color:#c20003">Lot: '.$_GET['lot'].'</span></h4>';  
		       }
	}	   
else{		   
		...
		$WHERE = !(empty($WHERE)) ? implode(" AND ",$WHERE) : "1";
		$get_cars = aj_get("select * from $table where $WHERE  order by YEAR Limit $start, $perpage"); // 1=>debug
		$get_cars_counterss = aj_get("select * from $table where $WHERE"); // 1=>debug  
...
<?php if($accepted == '0'){ ?> <span style="color:#ff0000; font-weight:bold;"><?php echo $bid_price.',000';?>&yen;</span> <?php  }?>
<?php if($accepted == '1'){ ?> <span style="color:#ff0000; font-weight:bold;"><?php echo $bid_price.',000';?>&yen;</span> <?php  }?>
...
<?php echo $row_bid['groupp'];?>
 ...
 <?php //echo $va['STATUS'];?>
<span style="color:#ffffff; font-weight:bold;"><?php //echo $va['STATUS'];?></span>
 
 
<?php if(strtolower($va['STATUS']) == 'sold'){ ?> <span style="color:#009900; font-weight:bold;"><?php echo $va['STATUS'];?></span>   <br/><?php  }?>
<?php if(strtolower($va['STATUS']) == 'sold by nego'){ ?> <span style="color:#009900; font-weight:bold;"><?php echo $va['STATUS'];?></span>   <br/><?php  }?>
<?php if(strtolower($va['STATUS']) == 'canceled'){ ?> <span style="color:#0066CC; font-weight:bold;"><?php echo $va['STATUS'];?></span>   <br/><?php  }?>
<?php if(strtolower($va['STATUS']) == 'not sold'){ ?> <span style="color:#0066CC; font-weight:bold;"><?php echo $va['STATUS'];?></span>   <br/><?php  }?>
